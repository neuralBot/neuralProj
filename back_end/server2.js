...
async.each(stock2, function (name, callback) {
  var urlstring = "http://www.setsmart.com/ism/historicalTrading.html?lstDisplay=T&symbol="+name+"&submit=go&lstMethod=AOM&submit.y=13&decorator=html&submit.x=24&showBeginDate=1%2F01%2F"+use_date[0]+"&lstPeriod=D&endDate=01%2F01%2F"+use_date[1]+"&beginDate=01%2F01%2F"+use_date[0]+"&quickPeriod=&showEndDate=01%2F01%2F"+use_date[1];
           ...
           $('script').filter(function(){
              var result = "";
                ...
                if(n!=-1)
                {
                  ...
                  var file_name = "test"+num_file;
                  num_file++;
                  /*fs.writeFile(file_name+'.json', JSON.stringify(res, null, 4), function(err){
...
//this
    if( (arr[i][5] < arr[i+1][5] ) && (arr[i+1][5] == arr[i+2][5])) // equaldown
    {
      ...
      for(j=0;j<=objpatt[1].total.length-1;j++)
      {
        //console.log(arr[i-1][7]+"SDSD"+objpatt[1].total[j][1]);
        if(objpatt[1].total.length-1 == 0 && j==0)
        {
          objpatt[1].total.push([1,arr[i-1][7]]); 
        }

        else if(arr[i-1][7] == objpatt[1].total[j][1])
        {
      ...
      {
        objpatt[1].total.push([1,arr[i-1][7]]); 
        //objpatt[1].total[objpatt[1].total.length] = objpatt[1].total[1,arr[i-1][7]];
      }
  ...
  objpatt[1].total.sort(sortFunction);
  console.log(objpatt[1].total);
}
